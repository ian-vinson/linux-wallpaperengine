...
// Per-vertex smooth normals, accumulated from adjacent face windings. MDLV's stored vertex
// format has 60 unused bytes per vertex beyond the confirmed position/uv fields (offsets 0 and
// 72 of an 80-byte stride) that plausibly hold a real normal, but that byte offset was not
// reliably confirmed against real files during scoping -- computing normals directly from the
// geometry itself sidesteps that uncertainty and is always correct for a static mesh.
...
MdlvParseResult WallpaperEngine::Data::Parsers::parseMdlv (const std::vector<char>& data) {
    ...
    const auto blocks = findMdlvMeshBlocks (reader, markerSize, endOffset, meshHeaderSize, vertexStride);
